...
May 13 2011 -	Prevent overwriting bounds of FranzCommandCallback[] array in addCommand()
...
Oct 2013 - FranzCommand object can be created using a SoftwareSerial object, for SoftwareSerial
...
***********************************************************************************/
#ifndef FranzCommand_h
#define FranzCommand_h
...
// If you want to use FranzCommand with the hardware serial port only, and want to disable
// SoftwareSerial support, and thus don't have to use "#include <SoftwareSerial.h>" in your
// sketches, then uncomment this define for FranzCommand_HARDWAREONLY, and comment out the 
...
//#define FranzCommand_HARDWAREONLY 1
#undef FranzCommand_HARDWAREONLY

#ifdef FranzCommand_HARDWAREONLY
#warning "Warning: Building FranzCommand without SoftwareSerial Support"
#endif
...
#define FranzCommandBUFFER 30
#define MAXFranzCommandS	30
#define MAXDELIMETER 2

#define FranzCommandDEBUG 1
#undef FranzCommandDEBUG      // Comment this out to run the library in debug mode (verbose messages)

class FranzCommand
{
	public:
		FranzCommand();      // Constructor
	...
	private:
		...
		char buffer[FranzCommandBUFFER];   // Buffer of stored characters while waiting for terminator character
		...
		typedef struct _callback {
			char command[FranzCommandBUFFER];
			void (*function)();
		} FranzCommandCallback;            // Data structure to hold Command/Handler function key-value pairs
		int numCommand;
		FranzCommandCallback CommandList[MAXFranzCommandS];   // Actual definition for command/handler array
		void (*defaultHandler)();           // Pointer to the default handler function 
		

};

#endif //FranzCommand_h